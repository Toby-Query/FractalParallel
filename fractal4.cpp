...
void kernel_omp ( unsigned char *ptr ){
    ...
    int colsOnThreads = DIM /threadsUsed;
    int lastCol=(threadID+1)*colsOnThreads;

    if(threadID==threadsUsed-1) lastCol=DIM;

    for (int y=0; y<DIM; y++) {
        for (int x=threadID*colsOnThreads; x<lastCol; x++) {
            int offset = x + y * DIM;
