...
void kernel_omp ( unsigned char *ptr ){
    ...
    #pragma omp  parallel
    {
    int threadID = omp_get_thread_num(), threadsUsed = omp_get_num_threads();
    int rowsOnThreads = DIM /threadsUsed;
    int lastRow=(threadID+1)*rowsOnThreads;

    if(threadID==threadsUsed-1) lastRow=DIM;

    for (int y=threadID*rowsOnThreads; y<lastRow; y++) {
        for (int x=0; x<DIM; x++) {
            int offset = x + y * DIM;
